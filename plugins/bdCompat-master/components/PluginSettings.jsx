const { React, getModuleByDisplayName } = require('Vendetta/webpack')
const { Modal } = require('Vendetta/components/modal')
const { close: closeModal } = require('Vendetta/modal')
...
try {
  ErrorBoundary = require('../../../src/Vendetta/coremods/settings/components/ErrorBoundary')
} catch (e) {
  console.error('Failed to load Vendetta\'s ErrorBoundary component!', e)
}
  ...
  renderPluginSettings() {
    ...
    try {
      ...
      const error = (e.stack || e.toString())
        ...
        .filter(
          (l) =>
            !l.includes('discordapp.com/assets/') && !l.includes('discord.com/assets/')
        )
        .join('\n')
      ...
      return (
        <div className='Vendetta-text Vendetta-settings-error'>
          <div>An error occurred while rendering the settings panel.</div>
          <code>{error}</code>
        </div>
      )
    }
