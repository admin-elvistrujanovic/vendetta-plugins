...
const { AddonAPI, BDApi, BDV2, ContentManager, VendettaPluginManager } = require('./modules');
const Settings = require('./components/Settings');

module.exports = class BDCompat extends Plugin {
  start() {
    this.loadStylesheet(__dirname + '/style.css');
    this.defineGlobals();

    this.registerSettings('bdCompat', {
      category: 'bdCompat',
  ...
  unload() {
    this.unregisterSettings('bdCompat');
  ...
  defineGlobals() {
    ...
    // Originally BdApi is an object, not a class
    window.BdApi = {};
    Object.getOwnPropertyNames(BDApi)
      .filter((m) => typeof BDApi[m] == 'function' || typeof BDApi[m] == 'object')
      .forEach((m) => {
  ...
  destroyGlobals() {
    ...
    globals.forEach((key) => delete window[key]);
